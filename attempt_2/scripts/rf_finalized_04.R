...
# set seed
set.seed(84732)
final_fit_rf <- fit(final_wflow_rf, train_upsampled)

# saving final fit
# save(final_fit_knn, file = here("attempt_2results/final_fit.rda"))
...
pred_rf <- game_test |>
  bind_cols(predict(final_fit_rf, new_data = game_test, type = "prob")) |>
  select(game_id, title, goty_won, predicted = .pred_1)


# save results
save(pred_rf, file = "attempt_2/final_results/pred_rf.rda")
